...
def set_critical(exceptions: Tuple[Exception, ...]) -> Exception:
    return Exception()


#TODO maybe this not needs the Report type with the boolean value, it's good enough with the exception type only.
def log_in(username: str, password: str) -> User:
    validation_list = [
        validator(username) for validator in validators.USERNAME_VALIDATORS
    ...
    if any(validator.event for validator in validation_list + [match]):
        raise set_critical(
            (validator.event for validator in validation_list if validator.event)
        )
